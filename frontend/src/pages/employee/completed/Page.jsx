  ...
  return (
    <div className={`${pageTheme.shell} min-h-screen`}>
      <SidebarProvider style={{ backgroundColor: "transparent" }}>
        <AppSidebar />
        <SidebarInset className={pageTheme.shell} style={{ backgroundColor: "transparent" }}>
          <header
          ...
          <main className="flex flex-1 flex-col gap-6 p-4 pt-6">
            <section className={`rounded-2xl border p-6 shadow-sm ${pageTheme.card}`}>
              <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                <div className="max-w-2xl space-y-4">
                  <div className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-sm font-medium ${pageTheme.muted}`}>
                    <BadgeCheck className="h-4 w-4" />
                    Completed work
                  </div>
                  <div className="space-y-2">
                    <h1 className={`text-3xl font-semibold tracking-tight ${pageTheme.cardTitle}`}>
                      Completed tasks
                    </h1>
                    <p className={`max-w-xl leading-7 ${pageTheme.cardText}`}>
                      Review finished tickets, confirm handoff quality, and keep a clean
                      record of work that has already been resolved.
                    </p>
                  </div>
                </div>
                ...
                <div className={`rounded-xl border px-4 py-3 text-sm ${pageTheme.border}`}>
                  ...
                  <p className={`mt-1 ${pageTheme.cardText}`}>Completed task queue</p>
                </div>
              </div>
            </section>
            ...
            <section className="grid gap-4 md:grid-cols-3">
              {[
                {
                  title: "Finished work",
                  body: "Tasks that are ready for audit, follow-up, or reporting.",
                },
                {
                  title: "Theme aware",
                  body: "The page keeps the sidebar and content shell in sync with the selected mode.",
                },
                {
                  title: "Ready for data",
                  body: "This area can be wired to a completed-task list or analytics panel next.",
                },
              ].map((item) => (
                <article key={item.title} className={`rounded-xl border p-4 shadow-sm ${pageTheme.card}`}>
                  <h2 className={`text-sm font-semibold uppercase tracking-wide ${pageTheme.cardTitle}`}>
                    {item.title}
                  </h2>
