...
export default function Page() {
  ...
  const debouncedSearch = useDebouncedValue(searchTerm, { lowercase: true });
  ...
  const {
    ...
    queryKey: ["showAssignedComplaints", "pending", debouncedSearch],
    queryFn: () => showAssignedComplaint({ search: debouncedSearch }),
  });
  ...
  const assignedTasks = useMemo(
    () => tasks.filter((task) => getStatusKey(task?.status) === "assigned"),
    [tasks],
  );
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (!needle) return assignedTasks;
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className={`mt-1 text-sm ${pageTheme.muted}`}>
                      Open an assigned task to review its customer, deadline,
                  ...
                  <p className={`text-sm ${pageTheme.muted}`}>
                    {isLoading
                      ? "Loading tasks"
                      : `${filteredTasks.length} of ${assignedTasks.length} shown`}
                  </p>
                </div>
              </div>
